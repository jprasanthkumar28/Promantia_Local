...
frappe.ui.form.on('Employee Deduction', {
	// this.frm.get_field("deduction_calculation").grid.cannot_add_rows = true;
});
	...
	"start_date": function(frm, cdt, cdn) {
    	...
    	if (row.deduction_type === "Onetime") {
			...
			frappe.call({
				...
				args: {
					doc: frm.doc.name, 
					d_type: row.deduction_type,
					s_date: row.start_date
				},
				async:false,
				callback: function(result) {
					// frappe.model.set_value("Deduction Details", doc, { row.end_date: result.message })
					console.log("Test");
